...
// Parses the pipeline YAML returned by the API and maps
// step types to icon names for the pipeline renderer.
// ─────────────────────────────────────────────
function parsePipelineYaml(yamlText: string): Array<{
  ...
  try {
    ...
    for (const raw of lines) {
      ...
      // Match steps: at any indent (handles pipeline:\n  steps:)
      ...
      // Step properties (strip leading `- ` in case model uses list style)
      ...
      if (clean.startsWith('type:')) {
        ...
        current.type = type;
        // Remap icon/color by type if id-based lookup was unknown
    ...
    // Flush last step if loop ended without flow:
...
export default function ChatPanel({ agentName, onToggleSidebar, isSidebarOpen = true }: ChatPanelProps) {
  ...
  const [generationStep, setGenerationStep] = useState<'config' | 'docker' | 'pipeline' | 'docs'>('config');
  ...
  // ── determine which step the user is requesting ────────────────────────────
  const detectStep = (text: string): 'config' | 'docker' | 'pipeline' | 'docs' => {
  ...
  // ── build assistant response per step ─────────────────────────────────────
  const buildAssistantMessage = (
    step: 'config' | 'docker' | 'pipeline' | 'docs',
    data: any,
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    try {
      ...
      // Always call API when: no cached data, or modify mode, or first config
      ...
      if (needsFreshData) {
        const res = await fetch('/api/generate', {
          ...
          body: JSON.stringify({
            ...
            previous: generatedData?.yaml ?? null,
          }),
        });
      ...
      setMessages(prev => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
          role: 'assistant',
          content: `Something went wrong while generating files. ${err instanceof Error ? err.message : ''}`.trim(),
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
      ]);
    }
              ...
              <React.Fragment key={node.id}>
                <div
                  ...
                  onMouseEnter={e => {
                    ...
                    (e.currentTarget as HTMLDivElement).style.background = `${node.color}12`;
                  }}
                  onMouseLeave={e => {
                    (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)';
                    (e.currentTarget as HTMLDivElement).style.background = `${node.color}08`;
                  }}
                >
  ...
  // ── file content renderer (dispatch by language) ───────────────────────────
  const renderFileContent = (msg: Message) => {
    ...
    if (language === 'pipeline') {
      return renderPipelineVisual(content);
    }
  ...
  const renderInputArea = () => (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', paddingBottom: '6px' }}>
        {/* Voice icon */}
        <button
          style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '6px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
  ...
  return (
    <div
      className="app-chat"
      style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'transparent', overflow: 'hidden', minWidth: 0, position: 'relative' }}
    >
      ...
      {/* Header */}
      <div style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', gap: '12px', background: 'transparent', flexShrink: 0, borderBottom: 'none', zIndex: 10 }}>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          <button
            style={{ fontSize: '18px', fontWeight: 600, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px', background: 'transparent', border: 'none', cursor: 'pointer', padding: '6px 12px', borderRadius: '8px', transition: 'var(--transition-smooth)' }}
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '12px' }}>
          <button
            style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', color: 'var(--text-secondary)', fontSize: '13px', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', transition: 'all 0.2s' }}
      ...
      {messages.length === 0 ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 5, padding: '0 20px', width: '100%' }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '15px', marginBottom: '40px', fontWeight: 400 }}>
            Describe your system and I'll generate the full config stack.
          </p>
                    ...
                    {msg.file && (
                      <div className="glass-panel" style={{ marginTop: '16px', borderRadius: '10px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            {msg.file.language === 'markdown' && (
                              <div style={{ display: 'flex', gap: '4px', marginLeft: '12px', background: 'rgba(0,0,0,0.3)', borderRadius: '6px', padding: '3px' }}>
                                {(['code', 'preview'] as const).map(mode => (
                                  <button
                                    key={mode}
                                    onClick={() => setMarkdownMode(prev => ({ ...prev, [msg.id]: mode }))}
                                    style={{
                                      background: (markdownMode[msg.id] ?? 'preview') === mode ? 'rgba(255,255,255,0.15)' : 'transparent',
                                      border: (markdownMode[msg.id] ?? 'preview') === mode ? '1px solid rgba(255,255,255,0.20)' : '1px solid transparent',
                                      color: (markdownMode[msg.id] ?? 'preview') === mode ? '#fff' : 'var(--text-muted)',
                          ...
                          {/* Copy button — not shown for pipeline or image */}
...
// ─────────────────────────────────────────────
// Icon helpers (keep bundle minimal)
// ─────────────────────────────────────────────
...
function AgentIconBtn({ emoji, label, onClick }: { emoji: string; label: string; onClick?: () => void }) {
  return (
    <button
      title={label}
      onClick={onClick}
      style={{ width: '30px', height: '30px', background: 'transparent', border: '1px solid transparent', borderRadius: '7px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
      onMouseEnter={e => { const b = e.currentTarget as HTMLButtonElement; b.style.background = 'rgba(124, 58, 237, 0.12)'; b.style.borderColor = 'var(--border-subtle)'; }}
